...
namespace Sit_In_Monitoring
{
   //MARZO
    public partial class SitInMonitoringForm : Form
    {
        ...
        readonly SqlConnection conn = new SqlConnection("Data Source=DNTS-MANOY\\SQLEXPRESS;Initial Catalog=DNTS_STUDENTS_RECORDS;Integrated Security=True;");
        ...
        public void AddStudent()
        {
            ...
            try
            {
                if (dt.Rows.Count == 0)
                {
                    ...
                    {
                        SqlCommand cmd = new SqlCommand("INSERT INTO Students VALUES(@studentId,@firstName, @middleInitial,@lastName,@section, @remainingTime, @CARDNO);", conn);
        ...
        /// <summary>
        /// Checks for basic SQL Injection attack attempt |> 
        /// </summary>
        ...
        public void StudentExisted()
        {
            ...
            if (txtStudentID.Text != "")
            {
                ...
                SqlDataAdapter s = new SqlDataAdapter("" +
                    "SELECT * " +
                    "FROM Students " +
                    "WHERE studentid = '" + txtStudentID.Text.ToString() + "'" +
                    "OR CARDNO = '" + txtStudentID.Text.ToString() + "'", conn);
                ds.Clear();
                s.Fill(ds);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        if(string.IsNullOrEmpty(txtCardNo.Text) || txtCardNo.Text == 0.ToString())
                        {
                            MessageBox.Show("Please fill in the card number!");
        ...
        {
           //Second Fixed// Already on 1/5/2024 |-->  
            ...
            #region DisplayLogOutInformation |> 
            ...
            if (remainingTime > 0)
            {
                ...
                if (wew.Rows.Count == 0)
                {
                    ...
                    if (dt.Rows.Count >= 1)
                    {
                        ...
                        {
                            if (MessageBox.Show("Do you want to override student's time?", "Override?", MessageBoxButtons.YesNo) == DialogResult.Yes) // I don't think this works... 
                                AddStudent();
                            else
                                ClearInputTextBoxes();
                        }
        ...
        public void EditStudentDetails()//BUG - AFTER EDITING THE TEXT THAT WAS PREVIOUSLY PUT DOESN'T CLEAR-- MUST BE FIX. ALSO DISABLE THE STUDENT ID EDIT. STUDENT ID IS NOT EDITABLE. // FIXED |> 
        {
            if (conn.State == ConnectionState.Closed)
                OpenSQL();
            ...
            // Don't worry about this part if code is unreadable for you |> 
            ...
            // Short sumamry, string value is returned after checking if string is empty or not, if empty then return current existing string, if input detected then return input as new string;) hekhok |>
        ...
        #region Bad Input |> Behavior UI - 
        private void CheckForBadInput(object sender, KeyPressEventArgs e) =>
            e.Handled = ((char.IsLetter(e.KeyChar) || !char.IsDigit(e.KeyChar)) && !char.Equals('-', e.KeyChar)) && !char.IsControl(e.KeyChar);
        ...
        private void TextBoxFocus(object sender, EventArgs e) // 
        {
            ...
            TextBox_Behavior(txtCardNo, mrg4, label22);
        ...
        private void KeyIsDown(object sender, KeyEventArgs e)
        {
            ...
            // Tab Switch Code -  // HEHEHEHEHEHEHEHE MUGANA
        ...
        private void BtnDelete_Click(object sender, EventArgs e)
        {
            ...
            //ShowAdminPasswordInput();
            MessageBox.Show("WALA NAMO GI BUTNGAN UG FUNCTIONALITY KAY ANA SI RHODNEY DILI NECESSARY MANG DELETE UG DATA..... SA DATABASE NALANG MANG DELETE HIHI");
        }
        ...
        {
            #region Display and Get Records |> 
            try
            {
        ...
        #region UI Behaviour NEW |> Edit Panel - 
        ...
        private void cbxSelectForm_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cbxSelectForm.Text.ToLower() == "student specific report")
            {
                ...
                //    "SELECT studentId" +
                //    "FROM Students";
                string SqlCommand = "SELECT studentId FROM Students";
        ...
        #region Can select custom records |> 
        ...
        #region General Function For Print Options |> 
        ...
        // Semestral Report Manager for Semestral Records in Database |> 
        ...
        private void btnAddSemesterToRecords_Click(object sender, EventArgs e)
        {
            ...
            if (case1 && case2 && case3)
            {
                ...
                string checkQuery = "SELECT COUNT(*) FROM SemestersAdded WHERE semester = @semester";
                SqlCommand checkCmd = new SqlCommand(checkQuery, conn);
                checkCmd.Parameters.AddWithValue("@semester", txtSemesterNameInput.Text);

                int exists = (int)checkCmd.ExecuteScalar();
                if (exists > 0)
                {
                    MessageBox.Show("The semester already exists. Please enter a unique semester name.",
                                    "Duplicate Entry", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                {
                    string AddSemester = "INSERT INTO SemestersAdded (semester, startOfSem, endOfSem) " +
                                         "VALUES (@semester, @start, @end)";
                    SqlCommand comm = new SqlCommand(AddSemester, conn);
                    comm.Parameters.AddWithValue("@semester", txtSemesterNameInput.Text);
                    comm.Parameters.AddWithValue("@start", dateTimePickerForStartOfSem.Value.Date);
                    comm.Parameters.AddWithValue("@end", dateTimePickerForEndOfSem.Value.Date);

                    comm.ExecuteNonQuery();
                    DisplayInDataGridForSemestermanager();
                    CloseSQL();
                    MessageBox.Show("Semester successfully added!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
